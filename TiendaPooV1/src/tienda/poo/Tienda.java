...
public class Tienda {
    ...
    public Tienda(String nombreTienda) {
        this.nombre_tienda = nombreTienda;
    ...
    public String filtrarTorres(String patron) {
        ...
        for (Producto prd : this.almacen.values()) {
            if (prd.getNombre().toLowerCase().contains(patron.toLowerCase())) {
                res += prd.toString();
            }
        }
        if (res.equals("")) {
    ...
    private Producto buscarJuego(String nombre_producto) {
        return this.almacen.get(nombre_producto);
    }

    //Método añadir producto
    public void añadirTorre(int stock, String nombre, String fabricante, 
            String nombre_juegos, double precio, double coste_unitario, char categoria) {
        Producto nuevo, busqueda;
        busqueda = this.buscarJuego(nombre_juegos);
        if (busqueda == null) {
            nuevo = new Producto(stock,nombre,fabricante,nombre_juegos,precio, stock, categoria, fabricante);
            this.almacen.put(nombre_juegos, nuevo);
        } else {
